    ...
    <div className="flex flex-col w-full min-h-dynamic bg-[#F5F5F5]">
      ...
      <header className="px-4 pt-3 pb-2 bg-[#F5F5F5]">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-[18px] font-extrabold text-[#2B2B2B]">Professionnels</h1>
        </div>
        ...
        <button onClick={() => setShowLocationPicker(true)}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-gray-200 text-[11px] font-medium text-[#2B2B2B]/70 cursor-pointer active:scale-95 transition-transform">
      ...
      {/* Listing */}
      <section className="flex-1 px-4 pb-6 overflow-y-auto pt-2">
        {filteredCategories.length === 0 && query ? (
          <div className="flex flex-col items-center justify-center pt-12">
            <span className="text-[32px] mb-2">🔍</span>
            <p className="text-[13px] font-semibold text-[#2B2B2B]">Aucun résultat</p>
            <p className="text-[11px] text-gray-500 mt-0.5">Essayez d'autres mots-clés</p>
            <button onClick={() => { setQuery(""); setLocationFilter(""); }}
          ...
          <div className="space-y-5">
            {filteredCategories.map((cat) => {
              ...
              const visiblePros = isExpanded ? catPros : catPros.slice(0, 3);

              return (
                <div key={cat.id}>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-1.5">
                      <span className="text-[14px]">{CATEGORY_ICONS[cat.id]}</span>
                      <h2 className="text-[15px] font-bold text-[#2B2B2B]">{cat.name}</h2>
                      <span className="text-[11px] text-gray-400 font-medium">({catPros.length})</span>
                    </div>
                    {!isExpanded && catPros.length > 3 && (
