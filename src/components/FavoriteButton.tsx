...
export default function FavoriteButton({ item, className = "" }: Props) {
  ...
  return (
    <div
      ...
      className={`absolute top-2 right-2 z-10 w-8 h-8 rounded-full bg-cm-elevated/95 backdrop-blur-sm border border-cm-border shadow-sm flex items-center justify-center cursor-pointer active:scale-90 transition-transform ${className}`}
    >
