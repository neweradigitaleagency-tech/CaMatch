  ...
  return (
    <ProfileSection title="Services" subtitle={mode !== "client" ? `${services.length} service${services.length > 1 ? "s" : ""}` : undefined}>
      <div className="flex flex-col gap-1.5">
        {services.length === 0 && (
          <div className="px-4 py-3 bg-gray-50 rounded-[14px]">
          ...
          return (
            <div key={service.id}
              className={`flex items-center gap-3 px-4 py-3 rounded-[14px] border transition-all cursor-pointer
                ${selected && mode !== "owner" ? "bg-emerald-50 border-emerald-300 ring-1 ring-emerald-300" : "bg-white border-gray-100 hover:border-gray-200"}
                ${isOwnerEdit ? "cursor-default" : ""}`}
              onClick={() => !isOwnerEdit && onToggleService?.(service.id)}>
              <div className="w-9 h-9 rounded-[10px] bg-gray-50 flex items-center justify-center text-[13px] font-black text-gray-600 shrink-0 uppercase">
                {service.name.charAt(0)}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-black text-gray-900">{service.name}</p>
                <p className="text-[10px] font-semibold text-gray-400 mt-0.5">{service.description}</p>
              </div>
              {!isOwnerEdit && (
                <span className="text-[13px] font-black text-gray-900">{service.priceEstimateXOF.toLocaleString("fr-FR")} <span className="text-[9px] font-bold text-gray-400">F</span></span>
              )}
              {isOwnerEdit ? (
        ...
        {isOwnerEdit && (
          <button onClick={() => onAddService?.({
            ...
            className="flex items-center justify-center gap-2 w-full h-12 rounded-[14px] border-2 border-dashed border-gray-200 text-gray-400 text-[12px] font-black cursor-pointer active:scale-[0.99] hover:border-gray-300 hover:text-gray-600 transition-all mt-1.5">
