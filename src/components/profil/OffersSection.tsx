  ...
  return (
    <ProfileSection title="Offres spéciales" subtitle={isOwnerEdit ? `${offers.length} offre${offers.length > 1 ? "s" : ""}` : undefined}>
      <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-none snap-x snap-mandatory -mx-4 px-4">
        {offers.length === 0 && (
          <div className="flex-1 px-4 py-3 bg-cm-surface rounded-[14px] min-w-[200px]">
        ...
        {offers.map((offer) => {
          ...
          return (
            <div key={offer.id}
              className="snap-start shrink-0 w-[220px] bg-gradient-to-br from-cm-text to-cm-text/90 rounded-[20px] p-4 text-white overflow-hidden relative cursor-pointer active:scale-[0.98] transition-transform"
        ...
        {isOwnerEdit && (
          <button onClick={() => onAddOffer?.({
            ...
            className="snap-start shrink-0 w-[220px] rounded-[20px] border-2 border-dashed border-cm-border-soft flex flex-col items-center justify-center gap-1.5 text-cm-text-muted cursor-pointer active:scale-[0.98] hover:border-cm-border hover:text-cm-text-soft transition-all">
