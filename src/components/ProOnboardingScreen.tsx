    ...
    <div className="min-h-screen bg-brand-cream flex flex-col">
      ...
      <div className="px-5 pt-5 pb-2">
        <div className="flex items-center justify-between mb-2">
          <button onClick={stepIndex > 0 ? prev : onSkip} className="w-9 h-9 rounded-full bg-white flex items-center justify-center shadow-sm cursor-pointer">
            <ChevronLeft className="w-5 h-5 text-brand-forest" />
          </button>
          <span className="text-[10px] font-bold text-on-surface-variant">{Math.round(progress)}%</span>
        </div>
        ...
        <p className="text-[9px] text-on-surface-variant mt-2 font-medium uppercase tracking-wider">
          ...
          >
            ...
            {step === "phone" && (
              <div className="pt-6 space-y-4">
                ...
                <p className="text-xs text-on-surface-variant">Saisissez votre numéro WhatsApp pour recevoir les missions.</p>
              ...
              <div className="pt-6 space-y-4">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <div><p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Prénom</p><input value={data.firstName} onChange={(e) => setData((d) => ({ ...d, firstName: e.target.value }))} className={inputClass} placeholder="Jean" /></div>
                    <div><p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Nom</p><input value={data.lastName} onChange={(e) => setData((d) => ({ ...d, lastName: e.target.value }))} className={inputClass} placeholder="Kouamé" /></div>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider mb-2">Métier</p>
                    <div className="grid grid-cols-2 gap-2">
                      {CATEGORIES.map((cat) => (
                        <button
                          key={cat.id}
            ...
            {step === "service-area" && (
              <div className="pt-6 space-y-4">
                ...
                <p className="text-xs text-on-surface-variant">Définissez votre rayon d'action autour de Cocody, Abidjan.</p>
                <div className="bg-white p-5 rounded-2xl space-y-4">
                  ...
                  <div>
                    <p className="text-[9px] font-bold text-on-surface-variant mb-2">Rayon : {data.serviceRadiusKm} km</p>
                    ...
                    <div className="flex justify-between text-[8px] text-on-surface-variant mt-1"><span>1 km</span><span>15 km</span><span>30 km</span></div>
                  </div>
                  <div className="bg-pale-mint p-3 rounded-xl">
                    <Navigation className="w-4 h-4 text-brand-forest inline mr-1" />
                    <span className="text-[10px]">La localisation précise sera activée via le GPS de votre téléphone.</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {step === "pricing" && (
              <div className="pt-6 space-y-4">
                ...
                <p className="text-xs text-on-surface-variant">Fixez vos prix. Vous pourrez les modifier plus tard.</p>
                <div className="space-y-3">
                  <div className="bg-white p-4 rounded-2xl">
                    <p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Taux horaire</p>
                    <div className="flex items-center gap-2"><input type="number" value={data.hourlyRateXOF} onChange={(e) => setData((d) => ({ ...d, hourlyRateXOF: parseInt(e.target.value) || 0 }))} className="text-2xl font-extrabold bg-transparent outline-none w-full" min="5000" /><span className="text-xs font-bold text-on-surface-variant">F / heure</span></div>
                  </div>
                  <div className="bg-white p-4 rounded-2xl">
                    <p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider mb-1">Frais de déplacement</p>
                    <div className="flex items-center gap-2"><input type="number" value={data.travelFeeXOF} onChange={(e) => setData((d) => ({ ...d, travelFeeXOF: parseInt(e.target.value) || 0 }))} className="text-2xl font-extrabold bg-transparent outline-none w-full" min="0" /><span className="text-xs font-bold text-on-surface-variant">F</span></div>
                  </div>
                </div>
                ...
                >
                  {photoPreview ? (
                    ...
                    <div className="w-32 h-32 rounded-full bg-pale-mint border-2 border-dashed border-gray-300 flex flex-col items-center justify-center mx-auto hover:border-brand-lime transition-colors">
                      ...
                      <p className="text-[10px] text-secondary/60">Ajouter une photo</p>
                    </div>
                  )}
                </div>
