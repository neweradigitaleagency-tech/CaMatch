      ...
      <div className="space-y-3">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="bg-cm-elevated rounded-xl border border-cm-border p-4">
            <div className="h-4 bg-cm-surface/50 animate-pulse rounded w-3/4 mb-2" />
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[20px] font-bold text-cm-text">Produits</h1>
          <p className="text-[12px] text-cm-text-muted">{products?.length ?? 0} produits</p>
        </div>
      ...
      <div className="flex gap-2">
        ...
        <select value={filter} onChange={(e) => setFilter(e.target.value as typeof filter)}
          className="h-9 px-3 bg-cm-elevated border border-cm-border rounded-xl text-[12px] focus:outline-none cursor-pointer">
      ...
      {filtered.length === 0 ? (
        <div className="bg-cm-elevated rounded-xl border border-cm-border p-8 text-center">
          ...
          <p className="text-[14px] font-medium text-cm-text-muted mb-1">Aucun produit trouvé</p>
          <p className="text-[12px] text-cm-text-muted mb-4">{search ? "Essayez un autre terme de recherche" : "Commencez par ajouter votre premier produit"}</p>
          {!search && (
            <button onClick={() => navigate("/supplier/products/new")}
              className="h-9 px-4 bg-cm-text text-white text-[12px] font-bold rounded-xl cursor-pointer">
              Ajouter un produit
            </button>
          )}
        </div>
            ...
            <div key={product.id}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex gap-3 flex-1 min-w-0">
                  {product.images?.[0] && (
                    <div className="shrink-0 w-14 h-14 rounded-lg overflow-hidden border border-cm-border/40">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-[14px] font-semibold text-cm-text">{product.name}</p>
                      {isOnSale && (
                        <span className="px-1.5 py-0.5 bg-red-100 text-red-600 rounded text-[10px] font-bold">-{discountPct}%</span>
                      )}
                      {!product.isActive && (
                        <span className="px-1.5 py-0.5 bg-cm-surface rounded text-[10px] text-cm-text-muted">Inactif</span>
                      )}
                    ...
                    <div className="flex items-center gap-3 mt-2 flex-wrap">
                      {isOnSale ? (
                        <>
                          ...
                          <span className="text-[11px] text-cm-text-muted line-through">{formatXOF(product.supplierPrice)}</span>
                        </>
                      ) : (
                        <span className="text-[12px] font-medium text-cm-text-soft">{formatXOF(product.supplierPrice)}</span>
                      )}
                      <span className="text-[11px] text-cm-text-muted">→ {formatXOF(product.cmPrice)}</span>
                      {product.unlimitedStock ? (
                        <span className="text-[11px] text-green-600">Stock illimité</span>
                      ) : (
                        <span className={`text-[11px] ${product.availableStock <= product.lowStockThreshold ? "text-orange-600 font-medium" : "text-cm-text-muted"}`}>
                ...
                <button onClick={(e) => { e.stopPropagation(); toggleActive.mutate({ productId: product.id, isActive: !product.isActive }) }}
                  className={`shrink-0 w-8 h-8 flex items-center justify-center rounded-lg cursor-pointer transition-colors ${
                    product.isActive ? "text-green-600 hover:bg-green-50" : "text-cm-border-soft hover:bg-cm-surface"
                  }`}>
