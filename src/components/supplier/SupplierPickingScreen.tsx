...
const STATUS_COLORS: Record<PickingStatus, string> = {
  ...
  cancelled: "text-cm-text-muted bg-cm-surface",
}
      ...
      <div className="px-4 py-3 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <p className="text-[13px] text-cm-text font-medium">Commande #{list.orderId.replace("order-", "")}</p>
            <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${statusColor}`}>
              {STATUS_LABELS[list.status]}
            </span>
          </div>
          <div className="flex items-center gap-3 mt-0.5">
            <span className="text-[11px] text-cm-text-muted">{totalQty} article{totalQty > 1 ? "s" : ""}</span>
            {list.createdAt && <span className="text-[11px] text-cm-text-muted">{formatDate(list.createdAt)}</span>}
            {list.preparedBy && <span className="text-[11px] text-cm-text-muted">Préparé par {list.preparedBy}</span>}
          </div>
          {list.status !== "completed" && (
            <div className="mt-2 flex items-center gap-2">
              ...
              <span className="text-[10px] text-cm-text-muted font-medium">{pickedQty}/{totalQty}</span>
            </div>
          )}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={() => setExpanded(!expanded)}
            className="p-1.5 text-cm-border-soft hover:text-cm-text-muted cursor-pointer">
      ...
      {expanded && (
        <div className="border-t border-cm-border/40">
              ...
              return (
                <div key={item.id} className="px-4 py-2.5 flex items-center gap-3">
                  <div className={`w-6 h-6 rounded border-2 flex items-center justify-center shrink-0 ${
                    done ? "bg-cm-green border-cm-green" : "border-cm-border"
                  }`}>
                    {done && <CheckCircle2 className="w-3.5 h-3.5 text-white" />}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-[12px] text-cm-text font-medium truncate">{item.productName}</p>
                    <div className="flex items-center gap-3 mt-0.5">
                      <span className="text-[10px] text-cm-text-muted">{item.productReference}</span>
                      {item.storageLocation && (
                        <span className="flex items-center gap-0.5 text-[10px] text-cm-text-muted">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {item.unit && <span className="text-[10px] text-cm-text-muted">{item.unit}</span>}
                  </div>
                </div>
              )
            })}
          ...
          {list.status === "in_progress" && (
            <div className="border-t border-cm-border/40 px-4 py-2 flex justify-end">
...
export default function SupplierPickingScreen() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-[20px] font-bold text-cm-text">Préparation de commandes</h1>
        <p className="text-[12px] text-cm-text-muted mt-1">Listes de picking — préparez et emballez les commandes</p>
      </div>
        ...
        {filters.map((f) => (
          <button key={f.key} onClick={() => setFilter(f.key)}
            className={`px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-colors cursor-pointer ${
              filter === f.key ? "bg-cm-green text-white" : "bg-cm-elevated text-cm-text-muted border border-cm-border hover:bg-cm-surface"
            }`}>
            {f.label}
          </button>
        ))}
      ...
      {isLoading ? (
        ...
        <div className="bg-cm-elevated rounded-xl border border-cm-border p-12 text-center">
          ...
          <p className="text-[13px] text-cm-text-muted">Aucune liste de préparation</p>
          <p className="text-[11px] text-cm-text-muted mt-1">Les commandes validées apparaîtront ici</p>
        </div>
      ) : (
