...
const STATUS_DETAILS: Record<SupplierPaymentStatus, { icon: typeof CheckCircle; color: string; bg: string }> = {
  ...
  failed: { icon: XCircle, color: "text-gray-600", bg: "bg-gray-50" },
}
  ...
  if (!payment) {
    return (
      <div className="max-w-2xl mx-auto text-center py-12">
        ...
        <p className="text-[14px] text-gray-500">Paiement introuvable</p>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-[18px] font-bold text-gray-900">Paiement {payment.id}</h1>
          <p className="text-[12px] text-gray-500">Transaction {payment.providerTransactionId}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
        <h2 className="text-[13px] font-semibold text-gray-900">Détail du montant</h2>
        <div className="space-y-2 text-[12px]">
          <div className="flex justify-between">
            <span className="text-gray-500">Sous-total (articles)</span>
            <span className="text-gray-900">{formatXOF(payment.subtotal)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Frais de livraison</span>
            <span className="text-gray-900">{formatXOF(payment.deliveryCost)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Commission Ça Match ({payment.netAmount > 0 ? Math.round(payment.commission / (payment.netAmount + payment.commission) * 100) : 0}%)</span>
            <span className="text-red-600">-{formatXOF(payment.commission)}</span>
          </div>
          <div className="border-t border-gray-100 pt-2 flex justify-between font-bold text-[14px]">
            <span className="text-gray-900">Net perçu</span>
      ...
      {/* Payment info */}
      <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
        <h2 className="text-[13px] font-semibold text-gray-900">Informations de paiement</h2>
        <div className="grid grid-cols-2 gap-3 text-[12px]">
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-gray-500">Provider</p>
            <p className="text-gray-900 font-medium mt-0.5">{PROVIDER_LABELS[payment.provider]}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-gray-500">Transaction ID</p>
            <p className="text-gray-900 font-medium mt-0.5 break-all">{payment.providerTransactionId}</p>
          </div>
          {payment.refundReason && (
            <div className="col-span-2 bg-gray-50 rounded-lg p-3">
              <p className="text-gray-500">Motif de remboursement</p>
              <p className="text-gray-900 font-medium mt-0.5">{payment.refundReason}</p>
            </div>
          )}
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-gray-500">Date de création</p>
            <p className="text-gray-900 font-medium mt-0.5">{new Date(payment.createdAt).toLocaleString("fr-FR")}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-gray-500">Dernière mise à jour</p>
            <p className="text-gray-900 font-medium mt-0.5">{new Date(payment.updatedAt).toLocaleString("fr-FR")}</p>
          </div>
        </div>
      </div>
      ...
      {order && (
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          <h2 className="text-[13px] font-semibold text-gray-900">Commande associée</h2>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[14px] font-semibold text-gray-900">{order.id}</p>
              <p className="text-[12px] text-gray-500">{order.clientName}</p>
            </div>
            <button onClick={() => navigate(`/supplier/orders/${order.id}`)}
              className="flex items-center gap-1.5 h-8 px-3 bg-gray-900 text-white text-[11px] font-medium rounded-lg cursor-pointer hover:opacity-90">
          ...
          {order.items && (
            <div className="space-y-1">
              {order.items.map((item) => (
                <div key={item.id} className="flex justify-between text-[11px] text-gray-600 p-1.5 bg-gray-50 rounded-lg">
