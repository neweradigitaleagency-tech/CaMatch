    ...
    <div className="px-5 py-5 pb-32 space-y-5">
      ...
      <div className="bg-brand-forest text-white p-6 rounded-3xl space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-bold text-brand-lime bg-brand-lime/20 px-2.5 py-1 rounded-full">
        ...
        <div className="grid grid-cols-3 gap-2 pt-2 border-t border-white/10">
          <div className="text-center">
            <p className="text-[9px] text-white/50 uppercase tracking-wider">Aujourd'hui</p>
            <p className="text-sm font-bold text-brand-lime">+{stats.todayEarningsXOF.toLocaleString()}</p>
          </div>
          <div className="text-center border-x border-white/10">
            <p className="text-[9px] text-white/50 uppercase tracking-wider">Semaine</p>
            <p className="text-sm font-bold text-brand-lime">+{stats.weekEarningsXOF.toLocaleString()}</p>
          </div>
          <div className="text-center">
            <p className="text-[9px] text-white/50 uppercase tracking-wider">Mois</p>
      ...
      {/* Recent Transactions */}
      <div>
        <h4 className="font-sans text-xs font-extrabold uppercase tracking-wider mb-3 flex items-center gap-1.5">
        ...
        {earnings.length === 0 ? (
          ...
          <div className="space-y-2">
            {earnings.map((e) => (
              <div
                key={e.id}
                className="bg-white p-4 rounded-2xl shadow-premium border border-pale-mint/15 flex items-center justify-between"
              >
                <div className="flex items-center gap-3">
                  <div
                ...
                <>
                  ...
                  <div className="bg-white p-5 rounded-2xl space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-xs font-bold text-on-surface-variant shrink-0">F CFA</span>
                    </div>
                      ...
                      <button
                        ...
                        className={`flex-1 py-2 rounded-xl text-[10px] font-bold border transition-all cursor-pointer ${
                          parseInt(amount.replace(/\D/g, ""), 10) === q
                            ...
                            : "bg-white border-pale-mint/40 text-on-surface-variant hover:border-brand-lime"
                        }`}
                      >
                        {q.toLocaleString()}
                      </button>
...
function EarningStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1 ${c.classes}`}>
