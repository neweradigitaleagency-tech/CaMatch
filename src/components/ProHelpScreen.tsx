      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
        {CATEGORIES.map((cat) => (
          <button
            ...
            className={`shrink-0 px-3.5 py-1.5 rounded-full text-[10px] font-bold transition-all cursor-pointer ${
              activeCategory === cat.id ? "bg-brand-forest text-white" : "bg-white text-on-surface-variant border border-pale-mint/30"
            }`}
          >
            {cat.label}
          </button>
      ...
      <AnimatePresence mode="wait">
        {selectedFaq ? (
          <motion.div key="detail" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
            <button onClick={() => setSelectedFaq(null)} className="flex items-center gap-1 text-[10px] font-bold text-on-surface-variant cursor-pointer">
            ...
            <div className="bg-white p-5 rounded-3xl shadow-premium border border-pale-mint/15 space-y-3">
              ...
              <p className="text-xs text-on-surface-variant leading-relaxed">{selectedFaq.answer}</p>
            </div>
            <div className="bg-pale-mint p-4 rounded-2xl space-y-2">
              <p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider">Encore besoin d'aide ?</p>
              <div className="flex gap-2">
                <button className="flex-1 py-2.5 bg-white rounded-xl text-[10px] font-bold flex items-center justify-center gap-1 cursor-pointer hover:bg-brand-lime/20 transition-colors">
          ...
          <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-2">
            {filtered.length === 0 ? (
              <div className="bg-white p-6 rounded-3xl text-center">
                ...
                <p className="text-xs text-on-surface-variant">Aucun résultat pour votre recherche</p>
              </div>
            ) : (
              filtered.map((faq) => (
                <div
