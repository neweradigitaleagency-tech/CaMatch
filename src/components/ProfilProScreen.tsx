  ...
  return (
    <div className="flex flex-col w-full pb-8 relative">
      ...
      <header className="flex justify-between items-center px-4 py-3 sticky top-0 z-10 bg-brand-cream/90 backdrop-blur-md">
        ...
        <button className="w-12 h-12 flex items-center justify-center rounded-full bg-white text-secondary hover:bg-pale-mint transition-colors shadow-sm cursor-pointer active:scale-95 duration-150">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent flex flex-col justify-end p-5">
            <div className="flex items-end justify-between">
              <div>
                ...
                {vInfo && (
                  <span className={`inline-flex items-center gap-1 mt-1.5 text-caption font-medium uppercase tracking-wider ${vInfo.color}`}>
              ...
              <div className="bg-brand-lime px-2.5 py-1 rounded-full flex items-center gap-1 shadow-md">
                ...
                <span className="text-caption font-medium text-brand-forest">{currentRating} ({pro.reviewCount})</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="flex gap-2 overflow-x-auto pb-1 no-scrollbar px-4 mb-4">
        <div className="flex-none bg-white px-4 py-2 rounded-full flex items-center gap-1.5 shadow-sm border border-pale-mint/10">
          ...
          <span className="text-caption font-medium text-brand-forest whitespace-nowrap">{pro.completedInterventions}+ Missions</span>
        </div>
        <div className="flex-none bg-white px-4 py-2 rounded-full flex items-center gap-1.5 shadow-sm border border-pale-mint/10">
          <MapPin className="w-3.5 h-3.5 text-brand-forest" />
          <span className="text-caption font-medium text-brand-forest whitespace-nowrap">{pro.locationNeighborhood}</span>
        </div>
        <div className="flex-none bg-white px-4 py-2 rounded-full flex items-center gap-1.5 shadow-sm border border-pale-mint/10">
          <ShieldCheck className="w-3.5 h-3.5 text-brand-forest" />
          <span className="text-caption font-medium text-brand-forest whitespace-nowrap">Garantie 7j</span>
        </div>
        <div className="flex-none bg-white px-4 py-2 rounded-full flex items-center gap-1.5 shadow-sm border border-pale-mint/10">
          <Clock className="w-3.5 h-3.5 text-brand-forest" />
          <span className="text-caption font-medium text-brand-forest whitespace-nowrap">{pro.experienceYears} ans d'exp.</span>
        </div>
      </section>
      ...
      <section className="px-4 mb-4">
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-pale-mint/20">
          <h3 className="text-caption font-medium text-brand-forest uppercase tracking-wider mb-3 flex items-center gap-1.5">
            <DollarSign className="w-3.5 h-3.5" /> Tarifs
          </h3>
          <div className="flex gap-3">
            <div className="flex-1 bg-pale-mint/40 rounded-xl p-3 text-center">
              <p className="text-caption text-secondary uppercase tracking-wider font-bold">Taux horaire</p>
              <p className="text-lg font-extrabold text-brand-forest">{pro.hourlyRateXOF.toLocaleString("fr-FR")} <span className="text-caption font-medium">F</span></p>
            </div>
            <div className="flex-1 bg-pale-mint/40 rounded-xl p-3 text-center">
              <p className="text-caption text-secondary uppercase tracking-wider font-bold">Déplacement</p>
              <p className="text-lg font-extrabold text-brand-forest">5 000 <span className="text-caption font-medium">F</span></p>
            </div>
            <div className="flex-1 bg-brand-forest rounded-xl p-3 text-center">
              <p className="text-caption text-white/60 uppercase tracking-wider font-bold">Forfait 2h</p>
              <p className="text-lg font-extrabold text-brand-lime">{(pro.hourlyRateXOF * 2 + 5000).toLocaleString("fr-FR")} <span className="text-caption font-medium">F</span></p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-caption text-secondary italic">Sélectionnez pour estimer</span>
        </div>
            ...
            return (
              ...
              >
                <div className="flex-1 pr-3">
                  ...
                  <p className="text-xs font-bold text-brand-forest/90">{service.priceEstimateXOF.toLocaleString("fr-FR")} F</p>
                  <p className="text-caption text-secondary italic mt-0.5">{service.description}</p>
                </div>
                <button
      ...
      <section className="px-4 mt-4">
        <div className="bg-pale-mint/40 p-4 rounded-2xl border border-pale-mint/10">
          <h3 className="text-caption font-medium text-brand-forest uppercase tracking-wider mb-1.5">À propos</h3>
          <p className="text-body-sm text-secondary leading-relaxed font-medium">{pro.bio || "Aucune bio pour le moment."}</p>
        </div>
      </section>
      ...
      <section className="px-4 mt-4 mb-8">
        <button
          ...
          className="w-full py-4 bg-brand-lime hover:brightness-105 active:scale-[0.98] transition-all rounded-2xl flex items-center justify-center gap-2 shadow-lg cursor-pointer text-brand-forest font-extrabold"
        >
