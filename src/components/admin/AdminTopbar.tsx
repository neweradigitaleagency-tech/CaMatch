...
export default function AdminTopbar({ onMenuToggle }: AdminTopbarProps) {
  ...
  return (
    <header className="bg-white border-b border-[var(--admin-border)] h-[60px] sticky top-0 z-30 flex items-center px-4 lg:px-6 gap-4">
      <button onClick={onMenuToggle} className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-gray-100 cursor-pointer lg:hidden">
      ...
      <div className="flex items-center gap-2">
        <button className="relative w-9 h-9 flex items-center justify-center rounded-lg hover:bg-gray-100 cursor-pointer">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 h-9 px-2 rounded-lg hover:bg-gray-100 cursor-pointer"
          >
            ...
            <>
              ...
              <div className="absolute right-0 top-full mt-1 w-52 bg-white border border-gray-200 rounded-xl shadow-lg z-20 py-1">
                <div className="px-4 py-2 border-b border-gray-100">
                  <p className="text-[13px] font-semibold text-gray-900">{admin?.firstname} {admin?.lastname}</p>
                  <p className="text-[11px] text-gray-500">{admin?.email}</p>
                </div>
                {admin?.roles.map((role) => (
                  <div key={role.id} className="px-4 py-1.5 text-[11px] text-gray-500">
