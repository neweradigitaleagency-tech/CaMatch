  ...
  return (
    <div className="flex flex-col w-full min-h-screen bg-brand-cream pb-32">
      <header className="flex items-center justify-between px-4 py-3 bg-brand-cream/90 backdrop-blur-md sticky top-0 z-10">
        ...
        <button onClick={onShowQR} className="w-12 h-12 flex items-center justify-center rounded-full bg-brand-lime text-brand-forest cursor-pointer active:scale-95">
      ...
      <div className="px-4 pt-4 space-y-2">
        <div className="bg-gradient-to-br from-brand-forest to-brand-forest/90 rounded-2xl p-5 shadow-premium">
          <p className="text-caption text-white/60 font-bold uppercase tracking-wider">Solde disponible</p>
          <p className="text-3xl font-extrabold text-white mt-1">{summary.availableBalanceXOF.toLocaleString()} F</p>
          <div className="flex gap-2 mt-3">
            <button onClick={onWithdraw} className="flex-1 h-10 bg-white/15 rounded-xl text-white text-caption font-medium flex items-center justify-center gap-1.5 cursor-pointer active:scale-95 hover:bg-white/20 transition-all">
              <ArrowUpRight className="w-3.5 h-3.5" /> Retirer
            </button>
            <button onClick={onShowQR} className="flex-1 h-10 bg-brand-lime rounded-xl text-brand-forest text-caption font-medium flex items-center justify-center gap-1.5 cursor-pointer active:scale-95 hover:brightness-105 transition-all">
        ...
        <div className="flex gap-2">
          <div className="bg-white rounded-2xl p-3 border border-pale-mint/20 shadow-sm flex-1">
            <p className="text-caption text-secondary">En attente</p>
            <p className="text-sm font-extrabold">{summary.pendingBalanceXOF.toLocaleString()} F</p>
          </div>
          <div className="bg-white rounded-2xl p-3 border border-pale-mint/20 shadow-sm flex-1">
            <p className="text-caption text-secondary">Ce mois</p>
      ...
      <div className="px-4 mt-4 flex gap-1.5">
        {(["all", "completed", "pending"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-3 py-1.5 rounded-xl text-caption font-medium cursor-pointer active:scale-95 transition-all ${
            ...
            <div key={tx.id} className="bg-white rounded-2xl p-3.5 border border-pale-mint/20 shadow-sm flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <p className="text-caption text-secondary">{new Date(tx.createdAt).toLocaleDateString("fr-FR")}</p>
                  <div className={`w-1 h-1 rounded-full ${STATUS_STYLES[tx.status]?.split(" ")[0] || "bg-secondary"}`} />
                  <span className={`text-caption px-1.5 py-0.5 rounded-full font-bold ${STATUS_STYLES[tx.status] || "bg-secondary/10 text-secondary"}`}>
              ...
              <div className="text-right">
                ...
                <p className="text-caption text-secondary/60">{100 - tx.commissionPercent}% net</p>
              </div>
            </div>
          );
        })}
