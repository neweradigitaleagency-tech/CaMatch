...
import { ArrowLeft, Star, MapPin, Clock, Briefcase, Phone, MessageSquare, Check, Edit3 } from "lucide-react";
    ...
    <div className="flex flex-col w-full min-h-screen bg-brand-cream pb-32">
      ...
      <header className="relative h-48 bg-gradient-to-br from-brand-forest to-brand-lime/80 overflow-hidden">
        ...
        <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-4 py-3 z-10">
          <button onClick={onBack} className="w-9 h-9 flex items-center justify-center rounded-full bg-white/20 text-white backdrop-blur-sm cursor-pointer active:scale-95">
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div className="flex gap-2">
            {isOwnProfile && onEdit && (
              <button onClick={onEdit} className="h-9 px-3 flex items-center gap-1.5 rounded-full bg-white/20 text-white backdrop-blur-sm text-[10px] font-bold cursor-pointer active:scale-95">
        ...
        <div className="absolute -bottom-12 left-4 flex items-end gap-4">
          <div className="w-24 h-24 rounded-2xl overflow-hidden border-[3px] border-brand-cream z-10 shadow-premium">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex items-center gap-3 mt-1.5">
              <div className="flex items-center gap-1">{renderStars(proRating)}<span className="text-[10px] text-secondary ml-1">({proReviews})</span></div>
              <span className="text-[10px] text-secondary flex items-center gap-1"><Briefcase className="w-3 h-3" />{proMissions} missions</span>
            </div>
      ...
      {proBio && (
        <div className="px-4 mb-4">
          <h3 className="text-[10px] uppercase tracking-widest text-secondary font-bold mb-1.5">À propos</h3>
          <p className="text-xs text-secondary leading-relaxed">{proBio}</p>
          <p className="text-[9px] text-secondary/50 mt-1">Membre depuis {proJoined}</p>
        </div>
      )}
      ...
      {portfolio.length > 0 && (
        <div className="px-4 mb-5">
          <h3 className="text-[10px] uppercase tracking-widest text-secondary font-bold mb-2">Portfolio ({portfolio.length})</h3>
          <div className="grid grid-cols-3 gap-1.5">
            {portfolio.map((img, i) => (
              <button key={img.id} onClick={() => { setGalleryIdx(i); setGalleryOpen(true); }} className="aspect-square rounded-xl overflow-hidden bg-pale-mint cursor-pointer active:scale-95 transition-transform">
      ...
      {/* Pricing */}
      <div className="px-4 mb-5">
        <h3 className="text-[10px] uppercase tracking-widest text-secondary font-bold mb-2">Tarifs</h3>
      ...
      {/* Availability */}
      <div className="px-4 mb-5">
        <h3 className="text-[10px] uppercase tracking-widest text-secondary font-bold mb-2">Disponibilités</h3>
        <div className="bg-white rounded-2xl p-4 border border-pale-mint/20 shadow-sm">
            ...
            {DAY_LABELS_FR.map((d, i) => {
              ...
              return (
                <div key={d} className={`flex flex-col items-center gap-0.5 ${isWorking ? "" : "opacity-40"}`}>
                  <div className={`w-[38px] h-[38px] rounded-xl flex items-center justify-center text-[9px] font-bold ${
                    isWorking ? "bg-brand-lime/20 text-brand-forest" : "bg-pale-mint text-secondary/40"
                  }`}>
                    {d}
                  </div>
                  {isWorking && (
                    <span className="text-[7px] text-secondary/60 font-medium">{slot!.start.slice(0, 5)}</span>
                  )}
                </div>
              );
            })}
          ...
          {availability.holidays.length > 0 && (
            <div className="mt-3 bg-red-50 border border-red-200 rounded-xl p-2 text-center">
              <p className="text-[9px] font-bold text-red-600">Jours fériés : {availability.holidays.length} jour(s)</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Gallery modal */}
      <ImageViewer
        images={portfolio.map(p => ({ url: p.url, title: p.title }))}
