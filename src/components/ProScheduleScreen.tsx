    ...
    <div className="px-5 py-5 pb-32 space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold transition-all cursor-pointer ${showSettings ? "bg-brand-lime text-brand-forest" : "bg-pale-mint text-on-surface-variant"}`}
        >
        ...
        {showSettings && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
            <div className="bg-white p-5 rounded-3xl shadow-premium border border-brand-lime/30 space-y-4">
              ...
              {/* Recurring Hours */}
              <div>
                <h4 className="font-sans text-xs font-extrabold uppercase tracking-wider flex items-center gap-1.5 mb-3">
                  ...
                  {[0, 1, 2, 3, 4, 5, 6].map((day) => {
                    ...
                    return (
                      <div key={day} className="flex items-start gap-2">
                        <span className="w-8 text-[10px] font-bold text-on-surface-variant mt-2">{dayName}</span>
                        <div className="flex-1 space-y-1">
                          {daySlots.length === 0 ? (
                            <p className="text-[10px] text-secondary/60 italic">Indisponible</p>
                          ) : (
                            daySlots.map((slot, i) => (
                              <div key={i} className="flex items-center gap-1">
                                <input
                                  ...
                                  className="w-20 text-[10px] bg-pale-mint/30 rounded-lg px-2 py-1 outline-none"
                                />
                                <span className="text-[9px] text-secondary">à</span>
                                <input
                          ...
                          <button
                            ...
                            className="text-[9px] font-bold text-brand-lime flex items-center gap-0.5 cursor-pointer"
                          >
              ...
              <div className="border-t border-pale-mint/20 pt-3">
                  ...
                  <div className="space-y-1 mb-3">
                    {blockedDays.map((bd, i) => (
                      <div key={i} className="flex items-center justify-between bg-pale-mint/30 p-2 rounded-xl">
                        <div>
                          <span className="text-[10px] font-bold">{new Date(bd.date).toLocaleDateString("fr-FR")}</span>
                          <span className="text-[9px] text-on-surface-variant ml-2">{bd.reason}</span>
                        </div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 text-[10px] bg-pale-mint/30 rounded-xl px-3 py-2 outline-none"
                  />
                  <input
                  ...
                  <button
                    ...
                    className="w-9 h-9 rounded-xl bg-brand-lime/20 flex items-center justify-center cursor-pointer"
                  >
      ...
      <div className="bg-white p-4 rounded-3xl shadow-premium border border-pale-mint/15 space-y-4">
        <div className="flex items-center justify-between">
          <button
            onClick={prevMonth}
            className="w-9 h-9 rounded-full bg-pale-mint flex items-center justify-center hover:bg-brand-lime/30 transition-all cursor-pointer"
          >
        ...
        <div className="grid grid-cols-7 gap-1">
          {DAYS.map((d) => (
            <div key={d} className="text-center text-[9px] font-bold text-on-surface-variant uppercase tracking-wider py-1">
              {d}
            </div>
          ))}
        </div>
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold transition-all cursor-pointer ${
                availableSlots[selectedDate]
                  ...
                  : "bg-pale-mint text-on-surface-variant"
              }`}
            >
              {availableSlots[selectedDate] ? (
          ...
          {availableSlots[selectedDate] && (
            <div className="bg-white p-3 rounded-2xl shadow-premium border border-pale-mint/15 flex flex-wrap gap-2">
              {availableSlots[selectedDate].map((slot) => (
                <span
                  key={slot}
                  className="text-[10px] font-bold bg-brand-lime/20 text-brand-forest px-3 py-1.5 rounded-full"
                >
                  <Clock className="w-3 h-3 inline mr-1" />
                  {slot}
                </span>
              ))}
              <span className="text-[9px] text-on-surface-variant italic self-center ml-1">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {job.scheduledTime && (
                        <span className="text-[10px] font-bold text-brand-forest flex items-center gap-0.5 mt-0.5">
