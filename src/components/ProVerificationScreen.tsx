  ...
  return (
    <div className="px-5 py-5 pb-32 space-y-5">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="w-9 h-9 rounded-full bg-white flex items-center justify-center shadow-sm cursor-pointer">
      ...
      <div className="bg-brand-forest text-white p-5 rounded-3xl space-y-3">
        ...
        <div>
          <p className="text-[9px] text-white/50 uppercase tracking-wider font-bold">Niveau actuel</p>
            ...
            <div key={level.level} className="bg-white rounded-3xl shadow-premium border border-pale-mint/15 overflow-hidden">
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <h4 className="text-xs font-extrabold">{level.label}</h4>
                    {isUnlocked && <CheckCircle className="w-4 h-4 text-green-500" />}
                    {isCurrent && <Clock className="w-4 h-4 text-amber-500" />}
                  </div>
                  <p className="text-[10px] text-on-surface-variant mt-0.5">{level.description}</p>
                </div>
              ...
              {isExpanded && (
                <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} className="px-4 pb-4 space-y-3 border-t border-pale-mint/10 pt-3">
                  {level.level === "id" && (
                    <>
                      <p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider">Documents d'identité (CNI / Passeport)</p>
                      ...
                      {verification.cniStatus === "pending" && (
                        <div className="flex items-center gap-2 text-amber-600 bg-amber-50 p-3 rounded-xl">
                          ...
                          <p className="text-[10px] font-medium">Vérification en cours (24-48h)</p>
                        </div>
                      )}
                    </>
                  )}
                  ...
                  {level.level === "background" && (
                    <>
                      <p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider">Casier judiciaire + Référence de quartier</p>
                      <div className="bg-pale-mint/30 p-3 rounded-xl space-y-2">
                        <p className="text-[10px] text-on-surface-variant">Téléchargez votre extrait de casier judiciaire (PDF) ou demandez une référence de voisinage.</p>
                        <button
                      ...
                      {verification.backgroundStatus === "pending" && (
                        <div className="flex items-center gap-2 text-purple-600 bg-purple-50 p-3 rounded-xl">
                          <Clock className="w-4 h-4 shrink-0" />
                          <p className="text-[10px] font-medium">Vérification en cours (48-72h)</p>
                        </div>
                      )}
                    </>
                  )}
                  ...
                  {level.level === "certified" && (
                    <>
                      <p className="text-[9px] font-bold text-on-surface-variant uppercase tracking-wider">Certification métier</p>
                      <div className="bg-pale-mint/30 p-3 rounded-xl space-y-2">
                        <p className="text-[10px] text-on-surface-variant">Téléchargez votre diplôme ou certification professionnelle (CAP, BEP, licence, etc.)</p>
                        <div
                        ...
                        >
                          {certPreview ? (
                            ...
                            <>
                              ...
                              <p className="text-[10px] text-secondary/60">Ajouter un document</p>
                            </>
                          )}
                        </div>
                  ...
                  {level.level === "elite" && (
                    <div className="bg-red-50 p-4 rounded-xl text-center">
                      ...
                      <p className="text-[10px] text-red-600/70 mt-1">Atteignez 100 missions avec une note de 4.9+ et zéro litige pour débloquer ce niveau.</p>
    ...
    <div onClick={preview ? undefined : onPress} className={`relative rounded-xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer overflow-hidden h-32 ${preview ? "border-green-400" : "border-gray-300 hover:border-brand-lime"}`}>
      {preview ? (
        <>
          ...
          <div className="absolute bottom-1 left-1 bg-green-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-full">{label} ✓</div>
        ...
        <>
          ...
          <p className="text-[9px] text-secondary/60 font-medium">{label}</p>
        </>
      )}
    </div>
  );
    ...
    <div onClick={preview ? undefined : onPress} className={`relative rounded-xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer overflow-hidden h-32 ${preview ? "border-green-400" : "border-gray-300 hover:border-brand-lime"}`}>
      {preview ? (
        <>
          ...
          <div className="absolute bottom-1 left-1 bg-green-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-full">Selfie ✓</div>
          <button onClick={(e) => { e.stopPropagation(); onClear(); }} className="absolute top-1 right-1 w-6 h-6 rounded-full bg-black/40 flex items-center justify-center"><XSmall className="w-3 h-3 text-white" /></button>
        </>
      ) : (
        <>
          <Camera className="w-6 h-6 text-secondary/60 mb-1" />
          <p className="text-[9px] text-secondary/60 font-medium">Selfie de vérification</p>
        </>
      )}
    </div>
  );
