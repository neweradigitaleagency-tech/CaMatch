...
export default function AdminSupportPage() {
  ...
  const columns: Column<SupportTicket>[] = [
      ...
      render: (t) => (
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-mono text-cm-text-muted">{t.id}</span>
            {t.priority === "urgent" && <span className="w-1.5 h-1.5 rounded-full bg-red-500" />}
          </div>
          <p className="text-[13px] font-medium text-cm-text truncate mt-0.5">{t.subject}</p>
        </div>
      ),
    },
    {
      key: "client_name", label: "Client", sortable: true, width: "140px",
      render: (t) => <span className="text-[12px] text-cm-text-soft">{t.client_name || "—"}</span>,
    },
    {
      key: "priority", label: "Priorité", sortable: true, width: "100px",
      render: (t) => (
        <span className={`inline-flex items-center px-2 py-0.5 text-[11px] rounded-md border ${PRIORITY_STYLES[t.priority] ?? "text-cm-text-muted"}`}>
    ...
    },
    {
      ...
      render: (t) => <span className="text-[12px] text-cm-text-muted">{SUPPORT_CATEGORY_LABELS[t.category] ?? t.category}</span>,
    },
    {
      ...
      render: (t) => {
        ...
        return (
          <div className="flex flex-col">
            <span className="text-[12px] text-cm-text-muted">{format(new Date(d), "d MMM HH:mm", { locale: fr })}</span>
            {t.admin_name && <span className="text-[11px] text-cm-text-muted mt-0.5">{t.admin_name}</span>}
          </div>
        )
      },
    },
        ...
        (canReply || canClose) && t.status !== "closed" ? (
          <div className="relative">
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded-lg text-cm-text-muted hover:bg-cm-surface hover:text-cm-text-soft transition-colors cursor-pointer"
            >
            ...
            {openMenuId === t.id && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 z-20 w-44 bg-cm-elevated border border-cm-border rounded-xl shadow-lg py-1">
                  {canReply && (
                    <button onClick={() => navigate(`/admin/support/${t.id}`)} className="w-full px-3.5 py-2 text-[12px] text-left text-cm-text-soft hover:bg-cm-surface flex items-center gap-2 cursor-pointer">
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[20px] font-bold text-cm-text">Support</h1>
