...
export default function AdminPlansPage() {
  ...
  const columns: Column<Plan>[] = [
      ...
      render: (p) => (
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-cm-surface flex items-center justify-center">
            <Tag className="w-4 h-4 text-cm-text-muted" />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-[13px] font-medium text-cm-text">{p.name}</span>
              {p.badge && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 font-medium">{p.badge}</span>}
            </div>
            <p className="text-[11px] text-cm-text-muted">{p.description ?? "—"}</p>
          </div>
        </div>
      ),
    },
    {
      ...
      render: (p) => (
        <span className="text-[12px] font-medium text-cm-text-soft">{p.type === "CLIENT" ? "Client" : "Pro"}</span>
      ),
    },
    {
      key: "price_monthly", label: "Prix mensuel", sortable: true, width: "110px",
      render: (p) => (
        <span className="text-[13px] font-medium text-cm-text">{p.price_monthly === 0 ? "Gratuit" : formatXOF(p.price_monthly)}</span>
      ),
    },
    {
      key: "price_yearly", label: "Prix annuel", sortable: true, width: "110px",
      render: (p) => (
        <span className="text-[13px] font-medium text-cm-text">{p.price_yearly === 0 ? "Gratuit" : formatXOF(p.price_yearly)}</span>
      ),
    },
    {
      key: "order", label: "Ordre", width: "60px",
      render: (p) => <span className="text-[12px] text-cm-text-muted">{p.display_order}</span>,
    },
    {
      key: "trial_days", label: "Essai", width: "60px",
      render: (p) => <span className="text-[12px] text-cm-text-muted">{p.trial_days > 0 ? `${p.trial_days}j` : "—"}</span>,
    },
    {
      key: "recommended", label: "Recommandé", width: "90px",
      render: (p) => p.recommended ? <StatusBadge status="active" label="Oui" /> : <span className="text-[12px] text-cm-text-muted">Non</span>,
    },
    {
      key: "active", label: "Actif", width: "80px",
      render: (p) => (
        <button onClick={(e) => { e.stopPropagation(); handleToggleActive(p) }}
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-medium cursor-pointer transition-colors ${p.active ? "bg-[var(--admin-accent-soft)] text-[var(--admin-accent)]" : "bg-cm-surface text-cm-text-muted"}`}>
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[20px] font-bold text-cm-text">Plans</h1>
          <p className="text-[13px] text-cm-text-muted mt-0.5">{plans.length} plans ({plans.filter((p) => p.active).length} actifs)</p>
        </div>
      ...
      <div className="flex items-center gap-2">
        {[{ key: "all", label: "Tous" }, { key: "CLIENT", label: "Client" }, { key: "PRO", label: "Pro" }].map((f) => (
          <button key={f.key} onClick={() => setFilterType(f.key)}
            className={`px-3 py-1.5 rounded-lg text-[12px] font-medium whitespace-nowrap cursor-pointer transition-colors ${filterType === f.key ? "bg-cm-text text-white" : "bg-cm-elevated border border-cm-border text-cm-text-soft hover:bg-cm-surface"}`}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => !saving && setModalOpen(false)}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-medium text-cm-text-soft mb-1">Prix mensuel (F CFA)</label>
              <input type="number" value={form.price_monthly} onChange={(e) => setForm({ ...form, price_monthly: Number(e.target.value) })}
                className="w-full h-9 px-3 text-[13px] bg-cm-surface border border-cm-border rounded-lg outline-none text-cm-text focus:border-cm-border" />
            </div>
            <div>
              <label className="block text-[12px] font-medium text-cm-text-soft mb-1">Prix annuel (F CFA)</label>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-[12px] font-medium text-cm-text-soft mb-1">Badge</label>
              <input type="text" value={form.badge} onChange={(e) => setForm({ ...form, badge: e.target.value })}
                placeholder="POPULAIRE"
                className="w-full h-9 px-3 text-[13px] bg-cm-surface border border-cm-border rounded-lg outline-none text-cm-text focus:border-cm-border" />
            </div>
            <div>
              <label className="block text-[12px] font-medium text-cm-text-soft mb-1">Ordre d'affichage</label>
              <input type="number" value={form.display_order} onChange={(e) => setForm({ ...form, display_order: Number(e.target.value) })}
                className="w-full h-9 px-3 text-[13px] bg-cm-surface border border-cm-border rounded-lg outline-none text-cm-text focus:border-cm-border" />
            </div>
            <div>
              <label className="block text-[12px] font-medium text-cm-text-soft mb-1">Jours d'essai</label>
          ...
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-[12px] text-cm-text-soft">Recommandé</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
