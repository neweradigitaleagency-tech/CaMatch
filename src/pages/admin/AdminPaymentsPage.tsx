...
export default function AdminPaymentsPage() {
  ...
  const txnColumns: Column<Transaction>[] = [
      ...
      render: (t) => (
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${TYPE_STYLES[t.type] ?? "text-gray-500 bg-gray-100"}`}>
              {TXN_TYPE_LABELS[t.type] ?? t.type}
            </span>
          </div>
          <p className="text-[12px] text-gray-700 truncate mt-0.5">{t.description}</p>
        </div>
      ),
    },
    {
      ...
      render: (t) => (
        <div className="flex flex-col">
          {t.client_name && <span className="text-[12px] text-gray-700">{t.client_name}</span>}
          {t.pro_name && <span className="text-[11px] text-gray-400">{t.pro_name}</span>}
          {!t.client_name && !t.pro_name && <span className="text-[12px] text-gray-400">—</span>}
        </div>
      ),
    },
    {
      ...
      render: (t) => {
        ...
        return (
          <div className="flex flex-col">
            <span className={`text-[12px] font-medium ${isNegative ? "text-red-600" : "text-gray-900"}`}>
              {isNegative ? "-" : ""}{formatXOF(t.amount)}
            </span>
            {t.fee > 0 && <span className="text-[10px] text-gray-400">Frais: {formatXOF(t.fee)}</span>}
          </div>
        )
      },
    },
    ...
    },
    {
      ...
      render: (t) => <span className="text-[12px] text-gray-500">{TXN_METHOD_LABELS[t.payment_method] ?? t.payment_method}</span>,
    },
    {
      key: "created_at", label: "Date", sortable: true, width: "110px",
      render: (t) => (
        <span className="text-[12px] text-gray-500">{format(new Date(t.created_at), "d MMM HH:mm", { locale: fr })}</span>
      ),
    },
    {
  ...
  const payoutColumns: Column<PayoutItem>[] = [
    {
      ...
      render: (p) => <span className="text-[11px] font-mono text-gray-400">{p.id.slice(0, 8)}</span>,
    },
    {
      key: "payee_name", label: "Bénéficiaire", sortable: true, width: "150px",
      render: (p) => <span className="text-[12px] text-gray-700">{p.payee_name || "—"}</span>,
    },
    {
      key: "amount", label: "Montant", sortable: true, width: "110px",
      render: (p) => <span className="text-[12px] font-medium text-gray-900">{formatXOF(p.amount)}</span>,
    },
    {
      key: "method", label: "Méthode", sortable: true, width: "120px",
      render: (p) => <span className="text-[12px] text-gray-500">{TXN_METHOD_LABELS[p.method] ?? p.method}</span>,
    },
    {
      key: "status", label: "Statut", sortable: true, width: "100px",
      render: (p) => {
    ...
    },
    {
      key: "created_at", label: "Date", sortable: true, width: "110px",
      render: (p) => (
        <span className="text-[12px] text-gray-500">{format(new Date(p.created_at), "d MMM", { locale: fr })}</span>
      ),
    },
    {
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
            {TXN_FILTERS.map((f) => (
              <button key={f.key} onClick={() => setTxnFilter(f.key)}
                className={`px-3 py-1.5 rounded-lg text-[12px] font-medium whitespace-nowrap cursor-pointer transition-colors ${txnFilter === f.key ? "bg-gray-900 text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
                {f.label}
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
            {PAYOUT_FILTERS.map((f) => (
              <button key={f.key} onClick={() => setPayoutFilter(f.key)}
                className={`px-3 py-1.5 rounded-lg text-[12px] font-medium whitespace-nowrap cursor-pointer transition-colors ${payoutFilter === f.key ? "bg-gray-900 text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
                {f.label}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <h3 className="text-[13px] font-semibold text-gray-900">Synthèse financière</h3>
            </div>
            <div className="p-5">
              <table className="w-full text-[12px]">
                ...
                <tbody className="divide-y divide-gray-50">
                  ...
                  <tr className="border-t border-gray-200">
                    <td className="py-3 pr-4 text-gray-400 font-medium">Virements pros effectués</td>
                    <td className="text-right py-3 text-gray-900 font-medium">{formatXOF(stats?.total_payouts ?? 0)}</td>
                  </tr>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <h3 className="text-[13px] font-semibold text-gray-900 mb-3">Répartition par méthode</h3>
            ...
            <MethodBar method="cash" label="Espèces" color="bg-gray-500" />
...
function MethodBar({ method, label, color }: { method: string; label?: string; color: string }) {
  return (
    <div className="flex items-center gap-3 py-2">
      <span className="text-[12px] text-gray-600 w-32">{label ?? METHOD_LABELS[method] ?? method}</span>
...
function Row({ label, value, accent, danger, warning, bold }: { label: string; value: string; accent?: boolean; danger?: boolean; warning?: boolean; bold?: boolean }) {
  return (
    <tr>
      ...
      <td className={`text-right py-2.5 ${accent ? "text-[var(--admin-accent)]" : danger ? "text-[var(--admin-danger)]" : warning ? "text-amber-600" : "text-gray-900"} ${bold ? "font-bold" : "font-medium"}`}>{value}</td>
    </tr>
  )
}
...
function StatCard({ icon, label, value, accent, valueColor }: { icon: React.ReactNode; label: string; value: string; accent?: boolean; valueColor?: string }) {
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4">
      ...
      <p className={`text-[18px] font-bold ${valueColor ?? (accent ? "text-[var(--admin-accent)]" : "text-gray-900")}`}>{value}</p>
    </div>
  )
}
