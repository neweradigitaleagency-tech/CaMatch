  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate("/admin/support")} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-cm-surface cursor-pointer">
          <ArrowLeft className="w-4 h-4 text-cm-text-soft" />
        </button>
        <div>
          <h1 className="text-[20px] font-bold text-cm-text">Ticket {ticket.id.slice(0, 8)}</h1>
          <p className="text-[13px] text-cm-text-muted mt-0.5">{ticket.subject}</p>
        </div>
      </div>

      <div className="bg-cm-elevated border border-cm-border rounded-xl p-5">
        <div className="flex items-start justify-between mb-3">
          <h2 className="text-[16px] font-bold text-cm-text">{ticket.subject}</h2>
          <StatusBadge status={STATUS_STYLES[ticket.status] ?? "inactive"} label={SUPPORT_STATUS_LABELS[ticket.status] ?? ticket.status} size="md" />
        </div>
        <p className="text-[13px] text-cm-text-soft leading-relaxed mb-4">{ticket.description}</p>
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[12px] text-cm-text-muted">
        ...
        {!isClosed && canClose && (
          <div className="flex gap-2 mt-4 pt-4 border-t border-cm-border/40">
            ...
            <button
              ...
              className="flex items-center gap-1.5 h-8 px-3.5 rounded-lg text-[12px] font-medium text-cm-text-soft bg-cm-surface border border-cm-border hover:bg-cm-surface cursor-pointer disabled:opacity-50"
            >
      ...
      <div className="bg-cm-elevated border border-cm-border rounded-xl">
        <div className="px-4 py-3 border-b border-cm-border/40 flex items-center justify-between">
          <h3 className="flex items-center gap-2 text-[13px] font-semibold text-cm-text"><MessageSquare className="w-4 h-4" /> Messages</h3>
          <span className="text-[11px] text-cm-text-muted">{regularMessages.length} message{regularMessages.length !== 1 ? "s" : ""}</span>
        </div>
        <div className="divide-y divide-cm-border/40 max-h-[500px] overflow-y-auto">
          {regularMessages.length === 0 ? (
            <div className="px-4 py-8 text-center text-[12px] text-cm-text-muted">Aucun message pour le moment</div>
          ) : (
            regularMessages.map((msg) => (
              <div key={msg.id} className="px-4 py-3">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white ${msg.sender_id === ticket.client_id ? "bg-blue-500" : "bg-cm-text"}`}>
                      {msg.sender_name.charAt(0)}
                    </div>
                    <span className="text-[12px] font-medium text-cm-text">{msg.sender_name || "Système"}</span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full border ${msg.sender_id === ticket.client_id ? "text-blue-600 bg-blue-50 border-blue-200" : "text-cm-text-soft bg-cm-surface border-cm-border"}`}>
                      {msg.sender_id === ticket.client_id ? "Client" : "Support"}
                    </span>
                  </div>
                  <span className="text-[11px] text-cm-text-muted">{format(new Date(msg.created_at), "d MMM HH:mm", { locale: fr })}</span>
                </div>
        ...
        {canReply && (
          <div className="px-4 py-3 border-t border-cm-border/40">
            <div className="flex items-center gap-2">
              <input
                type="text"
                value={replyText}
                ...
                className="flex-1 h-9 px-3 text-[13px] bg-cm-surface border border-cm-border rounded-lg outline-none text-cm-text placeholder:text-cm-text-muted focus:border-cm-border"
              />
              <button
                onClick={handleSendReply}
                disabled={sending || !replyText.trim()}
                className="h-9 px-3 bg-cm-text text-white text-[12px] font-medium rounded-lg hover:bg-cm-text/80 transition-colors flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
      ...
      <div className="bg-cm-elevated border border-cm-border rounded-xl">
        <button
          ...
          className="flex items-center justify-between w-full px-4 py-3 text-[13px] font-semibold text-cm-text cursor-pointer"
        >
          <span>Notes internes ({internalMessages.length})</span>
          <ChevronDown className={`w-4 h-4 text-cm-text-muted transition-transform ${showInternalNotes ? "rotate-180" : ""}`} />
        </button>
        {showInternalNotes && (
          <div className="px-4 pb-4 space-y-2">
            {internalMessages.length === 0 && (
              <p className="text-[12px] text-cm-text-muted py-2">Aucune note interne</p>
            )}
      ...
      {regularMessages.some((m) => m.file_urls && m.file_urls.length > 0) && (
        <div className="bg-cm-elevated border border-cm-border rounded-xl p-4">
          <h3 className="flex items-center gap-2 text-[13px] font-semibold text-cm-text mb-3"><Paperclip className="w-4 h-4" /> Pièces jointes</h3>
          <div className="space-y-2">
            {regularMessages.flatMap((m) => (m.file_urls ?? []).map((url, i) => (
              <div key={`${m.id}-${i}`} className="flex items-center justify-between p-3 rounded-lg bg-cm-surface">
                <div className="flex items-center gap-2">
                  <Paperclip className="w-3.5 h-3.5 text-cm-text-muted" />
                  <span className="text-[12px] text-cm-text-soft">{url.split("/").pop() || url}</span>
                </div>
  ...
  return (
    <div className="space-y-6 animate-pulse max-w-4xl">
      ...
      <div className="bg-cm-elevated border border-cm-border rounded-xl p-5 space-y-3">
        ...
        <div className="h-3 bg-cm-border-soft rounded w-3/4" />
        <div className="flex gap-4">
          <div className="h-3 bg-cm-border-soft rounded w-24" />
          <div className="h-3 bg-cm-border-soft rounded w-24" />
        </div>
      </div>
      <div className="bg-cm-elevated border border-cm-border rounded-xl p-4 space-y-3">
        ...
        <div className="h-12 bg-cm-border-soft rounded w-full" />
        <div className="h-12 bg-cm-border-soft rounded w-full" />
      </div>
    </div>
  )
