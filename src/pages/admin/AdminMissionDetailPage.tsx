...
const URGENCY_STYLES: Record<string, string> = { low: "text-gray-500 bg-gray-50 border-gray-200", medium: "text-amber-700 bg-amber-50 border-amber-200", high: "text-orange-700 bg-orange-50 border-orange-200", emergency: "text-red-700 bg-red-50 border-red-200 font-semibold" }
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate("/admin/missions")} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 cursor-pointer">
          <ArrowLeft className="w-4 h-4 text-gray-600" />
        </button>
        <div>
          <h1 className="text-[20px] font-bold text-gray-900">Mission {mission.id.slice(0, 8)}</h1>
          <p className="text-[13px] text-gray-500 mt-0.5">Détails, suivi et gestion de la mission</p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="text-[16px] font-bold text-gray-900">{getCategoryLabel(mission.category)}</h2>
            <p className="text-[12px] text-gray-500 mt-0.5">Créée le {format(new Date(mission.created_at), "d MMMM yyyy HH:mm", { locale: fr })}</p>
          </div>
        ...
        {mission.description && (
          <p className="text-[13px] text-gray-700 leading-relaxed mb-4">{mission.description}</p>
        )}
      ...
      {timeline.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <h3 className="text-[13px] font-semibold text-gray-900 mb-4 flex items-center gap-2"><Clock className="w-4 h-4" /> Chronologie</h3>
          <div className="relative pl-6 space-y-4">
            <div className="absolute left-[7px] top-1 bottom-1 w-0.5 bg-gray-200" />
            {timeline.map((t, i) => (
              <div key={t.id} className="relative">
                <div className={`absolute -left-[21px] top-1 w-3 h-3 rounded-full border-2 ${i === timeline.length - 1 ? "bg-[var(--admin-accent)] border-[var(--admin-accent)]" : "bg-white border-gray-300"}`} />
                <p className="text-[12px] font-medium text-gray-900">{t.event}</p>
      ...
      {conversation.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-[13px] font-semibold text-gray-900"><MessageSquare className="w-4 h-4" /> Conversation</h3>
            <span className="text-[11px] text-gray-400">{conversation.length} message{conversation.length !== 1 ? "s" : ""}</span>
          </div>
          <div className="divide-y divide-gray-50 max-h-[400px] overflow-y-auto">
            {conversation.map((msg) => (
              <div key={msg.id} className="px-4 py-3">
                ...
                <p className="text-[11px] text-gray-400 mt-1">{format(new Date(msg.created_at), "d MMM HH:mm", { locale: fr })}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Section title={<><FileText className="w-4 h-4" /> Notes</>}>
          {mission.client_notes ? (
            <div className="mb-3">
              <p className="text-[11px] text-gray-500 font-medium mb-1">Notes du client</p>
              <p className="text-[12px] text-gray-700 bg-gray-50 rounded-lg p-2.5">{mission.client_notes}</p>
            </div>
          ) : null}
          {mission.pro_notes ? (
            <div>
              <p className="text-[11px] text-gray-500 font-medium mb-1">Notes du professionnel</p>
              <p className="text-[12px] text-gray-700 bg-gray-50 rounded-lg p-2.5">{mission.pro_notes}</p>
            </div>
          ) : null}
          {!mission.client_notes && !mission.pro_notes && (
            <p className="text-[12px] text-gray-400 py-4 text-center">Aucune note</p>
          )}
        </Section>
      </div>
      ...
      {photos.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <h3 className="text-[13px] font-semibold text-gray-900 mb-4 flex items-center gap-2"><Image className="w-4 h-4" /> Photos {mission.before_photos?.length ? `(${displayPhotos.length}/${photos.length})` : ""}</h3>
          {mission.before_photos && mission.before_photos.length > 0 && (
            <div className="mb-3">
              <p className="text-[11px] text-gray-500 font-medium mb-2">Avant</p>
              <div className="flex gap-2 overflow-x-auto">
                {mission.before_photos.map((url, i) => (
                  <div key={i} className="w-24 h-24 shrink-0 rounded-lg overflow-hidden bg-gray-100 border border-gray-200">
          ...
          {mission.after_photos && mission.after_photos.length > 0 && (
            <div>
              <p className="text-[11px] text-gray-500 font-medium mb-2">Après</p>
              <div className="flex gap-2 overflow-x-auto">
...
function Section({ title, children }: { title: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4">
      <h3 className="text-[13px] font-semibold text-gray-900 mb-3 flex items-center gap-2">{title}</h3>
