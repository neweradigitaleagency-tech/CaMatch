  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-[20px] font-bold text-cm-text">Livraisons</h1>
          <p className="text-[13px] text-cm-text-muted mt-1">{deliveries.length} livraison(s)</p>
        </div>
      </div>
            ...
            <table className="w-full text-[12px]">
              <thead>
                <tr className="border-b border-cm-border/40 bg-cm-surface/50">
                  ...
                  <th className="text-left font-semibold text-cm-text-soft px-4 py-3">Adresse</th>
                  <th className="text-left font-semibold text-cm-text-soft px-4 py-3">Chauffeur</th>
                  <th className="text-left font-semibold text-cm-text-soft px-4 py-3">Véhicule</th>
                  <th className="text-left font-semibold text-cm-text-soft px-4 py-3">Étapes</th>
                  <th className="text-left font-semibold text-cm-text-soft px-4 py-3">Statut</th>
                  <th className="text-left font-semibold text-cm-text-soft px-4 py-3">Créée</th>
                ...
                {deliveries.map((d) => {
                  ...
                  return (
                    <tr key={d.id} className="border-b border-cm-border/40 hover:bg-cm-surface/50 transition-colors">
                      ...
                      <td className="px-4 py-3 font-medium text-cm-text">{d.city}</td>
                      <td className="px-4 py-3 text-cm-text-soft max-w-[200px] truncate">{d.address}</td>
                      <td className="px-4 py-3 text-cm-text-soft">{d.driverName ?? "—"}</td>
                      <td className="px-4 py-3 text-cm-text-soft max-w-[180px] truncate">{d.vehicleInfo ?? "—"}</td>
                      <td className="px-4 py-3 text-cm-text-muted">{d.stepCount}</td>
