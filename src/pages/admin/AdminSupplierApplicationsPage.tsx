...
export default function AdminSupplierApplicationsPage() {
  ...
  const appStatusColor = (s: string) => {
    const colors: Record<string, string> = {
      ...
      REJECTED: "bg-cm-surface text-cm-text-soft",
    }
    return colors[s] ?? "bg-cm-surface text-cm-text-soft"
  }

  return (
    <div className="space-y-4 animate-fade-in">
      <div>
        <h1 className="text-[20px] font-bold text-cm-text">Candidatures fournisseurs</h1>
        <p className="text-[13px] text-cm-text-muted mt-1">{apps.length} candidatures</p>
      </div>
      ...
      {loading ? (
        <div className="space-y-2">
          {[1, 2, 3].map((i) => <div key={i} className="h-16 bg-cm-elevated rounded-lg border border-cm-border animate-pulse" />)}
        </div>
      ) : apps.length === 0 ? (
        <div className="bg-cm-elevated rounded-xl border border-cm-border p-8 text-center">
          <p className="text-[14px] text-cm-text-muted">Aucune candidature</p>
        </div>
      ) : (
        <div className="space-y-2">
          {apps.map((app) => (
            <div key={app.id} className="bg-cm-elevated rounded-xl border border-cm-border p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="text-[14px] font-semibold text-cm-text">{app.companyName}</p>
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${appStatusColor(app.status)}`}>
                      {appStatusLabel(app.status)}
                    </span>
                  </div>
                  <p className="text-[12px] text-cm-text-soft mt-0.5">{app.ownerName} · {app.phone} · {app.city}</p>
                  {app.reviewNotes && <p className="text-[11px] text-cm-text-muted mt-1">Note : {app.reviewNotes}</p>}
                  <p className="text-[10px] text-cm-text-muted mt-1">{new Date(app.createdAt).toLocaleDateString("fr-FR")}</p>
                </div>
                {canReview && app.status !== "APPROVED" && app.status !== "REJECTED" && (
                  <div className="flex gap-1 shrink-0">
                    <button onClick={() => handleApprove(app.id)}
                      className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-green-50 text-cm-text-muted hover:text-green-600 cursor-pointer"
                      title="Approuver">
                      <CheckCircle className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleReject(app.id)}
                      className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-red-50 text-cm-text-muted hover:text-red-500 cursor-pointer"
                      title="Rejeter">
