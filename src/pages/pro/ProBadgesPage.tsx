    ...
    <div className="min-h-dynamic bg-[#F5F5F0]">
      ...
      <div className="w-full max-w-[448px] mx-auto px-4 pt-4 pb-24 space-y-4">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
          className="bg-white border border-gray-200 rounded-[20px] p-5 text-center shadow-sm">
          <div className={`w-16 h-16 rounded-full bg-gray-50 flex items-center justify-center mx-auto mb-2 text-3xl`}>
            {currentLevel.emoji}
          </div>
          <p className="text-[11px] text-gray-500 mb-0.5">Niveau actuel</p>
          <h2 className={`text-[22px] font-bold ${currentLevel.color}`}>{currentLevel.label}</h2>
          <div className="flex items-center justify-center gap-1 mt-1 text-[12px] text-gray-500">
          ...
          <div className="mt-4">
            ...
            <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-full bg-gray-900 rounded-full transition-all" style={{ width: `${Math.min(progress, 100)}%` }} />
            </div>
            {nextLevel ? (
              <p className="text-[11px] text-gray-500 mt-1">Prochain niveau: {nextLevel.emoji} {nextLevel.label} ({nextMinXP.toLocaleString("fr-FR")} XP)</p>
            ) : (
              <p className="text-[11px] text-gray-500 mt-1">Niveau maximum atteint</p>
            )}
          </div>
        </motion.div>

        {/* Niveaux */}
        <h2 className="text-[13px] font-bold text-gray-900">Niveaux</h2>
            ...
            return (
              <motion.div key={level.level} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                className={`bg-white border rounded-[14px] p-4 flex items-center gap-3 shadow-sm ${
                  isUnlocked ? "border-gray-200" : "border-gray-100 opacity-55"
                }`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg ${
                  isUnlocked ? "bg-gray-50" : "bg-gray-50"
                }`}>
                  {level.emoji}
                </div>
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <p className={`text-[13px] font-bold ${isUnlocked ? "text-gray-900" : "text-gray-400"}`}>
                      {level.label}
                    </p>
                    <span className="text-[10px] text-gray-500">{level.minXP.toLocaleString("fr-FR")}-{level.maxXP === Infinity ? "∞" : level.maxXP.toLocaleString("fr-FR")} XP</span>
                  </div>
                  <p className="text-[10px] text-gray-400 mt-0.5">Commission {level.commissionPercent}% · {level.benefits[0]}</p>
                  {isUnlocked && (
        ...
        {/* Badges */}
        <h2 className="text-[13px] font-bold text-gray-900 pt-2">Badges</h2>
          ...
          {MOCK_BADGES.map((badge, i) => (
            <motion.div key={badge.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.04 }}
              className={`bg-white border rounded-[14px] p-4 text-center shadow-sm ${
                badge.unlocked ? "border-gray-200" : "border-gray-100 opacity-55"
              }`}>
              <span className="text-2xl block mb-2">{badge.icon}</span>
              <p className={`text-[12px] font-bold ${badge.unlocked ? "text-gray-900" : "text-gray-400"}`}>{badge.name}</p>
              <p className="text-[10px] text-gray-500 mt-0.5">{badge.description}</p>
              {!badge.unlocked && <Lock className="w-3 h-3 text-gray-300 mx-auto mt-1" />}
